"""Force reset employee codes after legacy duplicates
...
def upgrade() -> None:
    ...
    # Temporary unique values prevent conflicts while replacing every code.
    for employee_id in changes:
        bind.execute(employees.update().where(employees.c.id == employee_id).values(code=f"TMP-RESET-{employee_id}"))
