...
test('période en jours : calendrier inclusif, validation et ligne ciblée', async () => {
  ...
  try {
    ...
    assert.match(d.getElementById('fact-days-error').textContent,/Complétez les deux dates/);
    to.value='2026-09-09';to.dispatchEvent(new w.Event('input'));
    d.getElementById('fact-days-form').dispatchEvent(new w.Event('submit',{cancelable:true}));
    assert.match(d.getElementById('fact-days-error').textContent,/postérieure/);
    assert.equal(row.querySelector('.fact-ligne-qte').value,'1');
    ...
    d.getElementById('fact-days-validate').click();
