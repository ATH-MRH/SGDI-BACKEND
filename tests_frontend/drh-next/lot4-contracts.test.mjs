...
test("aucun appel réseau tant qu'aucune recherche n'est tapée", async () => {
  ...
  assert.match(document.querySelector("#dn-contracts-results").textContent, /Tapez un nom/);
});
...
test("recherche : debounce ~300ms, un seul appel à /employees/page (jamais /contracts sans employee_id)", async () => {
  ...
  assert.match(document.querySelector("#dn-contracts-results").textContent, /BELAID|Belaid/i);
...
test("vider la recherche revient à l'état neutre sans appel réseau", async () => {
  ...
  await new Promise(r => setTimeout(r, 350));
  assert.match(document.querySelector("#dn-contracts-results").textContent, /B/);
  let calls = 0;
  window.fetch = async () => { calls++; return jsonResp({}); };
...
test("aucun résultat : état vide explicite, pas une erreur", async () => {
  ...
  await new Promise(r => setTimeout(r, 350));
  assert.doesNotMatch(document.querySelector("#dn-contracts-results").innerHTML, /dn-error-state/);
  assert.match(document.querySelector("#dn-contracts-results").textContent, /Aucun employé/);
});
...
test("course recherche : une frappe plus récente doit gagner sur une réponse tardive d'une frappe précédente", async () => {
  ...
  await new Promise(r => setTimeout(r, 350));
  assert.match(document.querySelector("#dn-contracts-results").textContent, /RAPIDE/);
  resolveSlow?.();
  await tick(); await tick();
  assert.doesNotMatch(document.querySelector("#dn-contracts-results").textContent, /LENT/);
});
