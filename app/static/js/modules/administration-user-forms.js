...
function adminPasswordFieldHTML(name,label,isNew,isDefined){
  ...
  return `<div class="admin-password-field"><label class="label" for="${id}">${label}${isNew?" *":""}</label><div class="admin-password-control"><input id="${id}" class="input" type="password" name="${name}" autocomplete="new-password" placeholder="${isNew?"Saisir un mot de passe":isDefined?"Inchangé":"Définir un mot de passe"}" aria-describedby="${id}-hint"/><button type="button" class="admin-password-toggle" title="Afficher le ${label.toLowerCase()}" aria-controls="${id}" aria-label="Afficher le ${label.toLowerCase()}" aria-pressed="false" data-password-label="${label.toLowerCase()}" onclick="adminTogglePasswordVisibility(this)"><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/><path class="admin-password-slash" d="m3 3 18 18"/></svg></button></div><small id="${id}-hint" class="admin-password-hint">${hint}</small></div>`;
}
