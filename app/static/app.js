(function () {
  ...
  function toast(message, type = "info") {
    if (typeof window.toast === "function" && window.toast !== toast) { window.toast(message, type); return; }
    const el = document.getElementById("toast-host") || document.getElementById("toast");
    if (!el) return;
  ...
  function renderLogin() {
    SGDI.logout();
    app.innerHTML = `<div style="min-height:100vh;background:linear-gradient(135deg,#f8fbff 0%,#edf5fb 52%,#f7faf9 100%);color:#0f172a;position:relative;overflow:auto">
      <div style="position:absolute;inset:0;pointer-events:none;background-image:linear-gradient(rgba(15,23,42,.055) 1px,transparent 1px),linear-gradient(90deg,rgba(15,23,42,.055) 1px,transparent 1px);background-size:44px 44px"></div>
      <main style="position:relative;width:min(1120px,92vw);min-height:100vh;margin:0 auto;padding:clamp(28px,6vh,72px) 0;display:grid;grid-template-columns:minmax(0,1.05fr) minmax(360px,.95fr);align-items:center;gap:clamp(28px,5vw,76px)">
        <section>
          <div style="font-size:clamp(72px,10vw,138px);line-height:.82;font-weight:950;letter-spacing:0;color:#07111f">ATLA<span style="color:#1267a8">S</span></div>
          <p style="margin:18px 0 0;max-width:430px;color:#476173;font-size:clamp(17px,2vw,24px);line-height:1.35;font-weight:850">Suite de gestion integree</p>
          <div style="width:min(360px,80%);margin-top:64px;padding:18px;border:1px solid rgba(15,23,42,.09);border-radius:8px;background:rgba(255,255,255,.78);box-shadow:0 24px 60px rgba(38,67,92,.16);backdrop-filter:blur(16px)">
            <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:16px;font-size:14px"><strong>Portail operationnel</strong><span style="color:#087f5b;font-size:12px;font-weight:900">En ligne</span></div>
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px">${["DRH","OPS","FIN","MAT"].map(x=>`<span style="height:64px;border-radius:7px;background:#edf5fb;color:#1267a8;display:flex;align-items:center;justify-content:center;font-weight:950">${x}</span>`).join("")}</div>
          </div>
        </section>
        <section style="width:100%;border:1px solid rgba(15,23,42,.1);border-radius:8px;background:rgba(255,255,255,.88);box-shadow:0 28px 70px rgba(31,50,71,.18);backdrop-filter:blur(18px);padding:clamp(28px,4vw,44px)">
          <span style="display:inline-flex;align-items:center;min-height:30px;padding:0 11px;border:1px solid rgba(18,103,168,.2);border-radius:999px;color:#1267a8;background:#eff7fd;font-size:12px;font-weight:950">Acces securise</span>
          <h1 style="margin:16px 0 8px;color:#0f172a;font-size:clamp(34px,4.2vw,52px);line-height:1;letter-spacing:0">Bienvenue</h1>
          <p style="margin:0 0 28px;color:#64748b;font-size:15px;line-height:1.55">Connectez-vous pour acceder a votre espace ATLAS.</p>
          <form onsubmit="event.preventDefault();login(this.username.value,this.password.value)" style="display:grid;gap:16px">
            <label style="display:grid;gap:7px;color:#475569;font-size:13px;font-weight:900">Identifiant
              <input style="width:100%;height:56px;border:1px solid #d9e3ec;border-radius:8px;background:#fff;color:#0f172a;padding:0 16px;font-size:17px;font-weight:850;outline:none" name="username" required autofocus/>
            </label>
            <label style="display:grid;gap:7px;color:#475569;font-size:13px;font-weight:900">Mot de passe
              <input style="width:100%;height:56px;border:1px solid #d9e3ec;border-radius:8px;background:#fff;color:#0f172a;padding:0 16px;font-size:17px;font-weight:850;outline:none" type="password" name="password" required/>
            </label>
            <button style="width:100%;height:56px;margin-top:6px;border:0;border-radius:8px;background:#0f172a;color:#fff;box-shadow:0 16px 34px rgba(15,23,42,.2);font-size:16px;font-weight:950;cursor:pointer">Se connecter</button>
          </form>
          <div style="display:flex;align-items:center;gap:9px;margin-top:14px;color:#64748b;font-size:13px;font-weight:850"><span style="width:9px;height:9px;border-radius:999px;background:#16a34a;box-shadow:0 0 0 5px rgba(22,163,74,.13)"></span> Session chiffree</div>
        </section>
      </main>
    </div>`;
  }
  ...
  window.openSiteForm = function () {
    openModal("Nouveau site", `<form id="siteForm" class="form-grid">
      ...
      ${selectField("Rotation",         "rotation_system",    ["24/48","3x8","1/3","1/2","1/1"],"24/48")}
  ...
  const _appRoutes = new Set(NAV.map(n => n.path));
  function _isAppRoute(hash) {
    const path = hash.replace(/^#/, "");
    return _appRoutes.has(path) || [..._appRoutes].some(p => path.startsWith(p + "/"));
  }
  window.renderRoute = renderRoute;
  window.addEventListener("hashchange", () => { if (_isAppRoute(location.hash)) renderRoute(); });
